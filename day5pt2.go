...
func main() {
	...
	for _, s := range input {
		...
		// Check if the source stack has enough items to move.
		if len(queues[from-1]) < n {
			n = len(queues[from-1])
		}
		
		// Move the specified number of items from one stack to another.
		copied := make([]string, n)
		copy(copied, queues[from-1][len(queues[from-1])-n:])
		queues[from-1] = queues[from-1][:len(queues[from-1])-n]
		queues[to-1] = append(queues[to-1], copied...)
